			  	...
			  	el.addEventListener('drop', function(e) {
					...
					el.classList.remove('open');
				  	...
				  	if (canvas.childNodes.length == 0) { 
				  		canvas.innerHTML +=  '<div data-colus="1" class="row-holder col-1"></div>'; } 
					else if (canvas.childNodes.length <= 4) {
						var row = canvas.querySelector('.row-holder').childNodes.length + 1;
						canvas.querySelector('.row-holder').dataset.colus = row;
					  	canvas.querySelector('.row-holder').className = 'row-holder col-'+ row;
					}
					else {
						cancel(e);
					  	return false;
					...
					document.querySelector('#image-'+e.dataTransfer.getData('id')).className = 'img-responsive used';
...
jQuery('.editor').on('input', function(e) {
...
[].forEach.call(
  ...
  function(el){
	el.addEventListener('dragstart', function(e) {
		...
		e.dataTransfer.setData('text', el.dataset.full);
